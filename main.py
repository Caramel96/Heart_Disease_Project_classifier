...
with open('Models/Tree_model.sav', 'rb') as f:
    randomforest = pickle.load(f)

def get_predictions(age,sex,cp,trestbps,chol,fbs,restecg,
                    thalach,exang,oldpeak,slope,ca,thal,req_model):
...
def my_form_post():
    if request.method == 'POST':
        ...
        target = get_predictions(age,sex,cp,trestbps,chol,fbs,restecg,
                    thalach,exang,oldpeak,slope,ca,thal, req_model)

        if target==1:
            sale_making = 'Possible heart Disease'
        else:
            sale_making = 'Possibly not Heart Disease'
